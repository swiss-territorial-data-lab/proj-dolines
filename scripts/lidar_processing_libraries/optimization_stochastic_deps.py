...
import assess_results, wbt_stochastic_depressions, merge_dem_over_aoi, post_processing
...
def objective(trial, dem_dir, dem_correspondence_df, aoi_gdf, ref_data_type, ref_data_gdf,
    ...
    autocorr_range_factor = trial.suggest_float('autocorr_range_factor', 2.5, 5, step=0.5)
    iterations = trial.suggest_int('iterations', 100, 300, step=50)
    threshold = trial.suggest_float('threshold', 0.75, 0.95, step=0.05)
    max_part_in_lake = trial.suggest_float('max_part_in_lake', 0.05, 0.35, step=0.05)
    max_part_in_river = trial.suggest_float('max_part_in_river', 0.05, 0.4, step=0.05)
    ...
    level_set_params = {
        'autocorr_range': autocorr_range_factor*resolution,
    ...
    detected_depressions_gdf, _ = wbt_stochastic_depressions.main(
        dem_list, non_sedimentary_gdf=non_sedimentary_areas_gdf, builtup_areas_gdf=builtup_areas_gdf, working_dir=working_dir, output_dir=output_dir, overwrite=True, **level_set_params
    )
    ...
    metric, _ = assess_results.main(ref_data_type, ref_data_gdf, detected_dolines_gdf, aoi_gdf, det_type='stochastic_depressions')
...
if study.best_value !=0:
    ...
    dem_list = glob(os.path.join(output_dir, 'merged_dems', '*.tif'))
    autocorr_range = study.best_params['autocorr_range_factor'] * study.best_params['resolution']
    detected_depressions_gdf, depression_files = wbt_stochastic_depressions.main(
        dem_list, autocorr_range, study.best_params['iterations'], study.best_params['threshold'], non_sedi_areas_gdf, builtup_areas_gdf,
        working_dir=WORKING_DIR, output_dir=output_dir, overwrite=True, save_extra=True
    )
    ...
    detected_dolines_gdf = assess_results.prepare_dolines_to_assessment(detected_dolines_gdf)
    metric, assessment_files = assess_results.main(REF_TYPE, ref_data_gdf, detected_dolines_gdf, aoi_gdf, det_type='stochastic_depressions', 
